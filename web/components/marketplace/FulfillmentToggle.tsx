      ...
      <div className={compact ? "flex gap-2" : "space-y-2"}>
        ...
        >
          ...
          <div className={`flex items-center ${compact ? "flex-col gap-1" : "gap-3 w-full"}`}>
            <div className={`rounded-lg p-1.5 shrink-0 ${value === "SELF_PICKUP" ? "bg-green-100" : "bg-gray-100"}`}>
              <Package size={compact ? 16 : 18} className={value === "SELF_PICKUP" ? "text-green-600" : "text-gray-400"} />
            </div>
            <div className={compact ? "text-center" : "flex-1"}>
              <p className={`font-semibold text-sm ${value === "SELF_PICKUP" ? "text-green-700" : "text-gray-700"}`}>
                {compact ? t("selfPickupCompact") : t("selfPickup")}
              </p>
              {!compact && <p className="text-xs text-gray-400 mt-0.5">{t("selfPickupDescription")}</p>}
              <p className={`text-xs font-medium mt-0.5 ${value === "SELF_PICKUP" ? "text-green-600" : "text-gray-400"}`}>
                {tc("free")}
              </p>
            </div>
        ...
        >
          ...
          />
          <div className={`flex items-center ${compact ? "flex-col gap-1" : "gap-3 w-full"}`}>
            <div className={`rounded-lg p-1.5 shrink-0 ${value === "DELIVERED" ? "bg-purple-100" : "bg-gray-100"}`}>
              <Truck size={compact ? 16 : 18} className={value === "DELIVERED" ? "text-purple-600" : "text-gray-400"} />
            </div>
            <div className={compact ? "text-center" : "flex-1"}>
              <p className={`font-semibold text-sm ${value === "DELIVERED" ? "text-purple-700" : "text-gray-700"}`}>
                {compact ? t("deliveryCompact") : t("delivery")}
              </p>
              {!compact && <p className="text-xs text-gray-400 mt-0.5">{t("deliveryDescription")}</p>}
              <p className={`text-xs font-medium mt-0.5 ${value === "DELIVERED" ? "text-purple-600" : "text-gray-400"}`}>
