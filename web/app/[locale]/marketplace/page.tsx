    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="sticky top-0 z-30 bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <span className="font-bold text-gray-900 text-lg hidden sm:block">{tc("appName")}</span>
            <span className="text-xs text-gray-400 hidden sm:block">{tn("marketplace")}</span>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              aria-label={tn("cartAriaLabel", { count: cartCount })}
            >
              <ShoppingCart size={16} />
              <span className="hidden sm:inline">{tn("orderCart")}</span>
              {cartCount > 0 && (
      ...
      <div className="bg-gradient-to-r from-green-700 to-emerald-500 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          <h1 className="text-2xl sm:text-3xl font-bold mb-1">{t("heroTitle")}</h1>
          <p className="text-green-100 text-sm sm:text-base max-w-xl">{t("heroSubtitle")}</p>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-4">
              ...
              {cartCount > 0 && (
                <button onClick={() => setShowCheckout(true)} className="flex items-center gap-2 text-sm text-green-700 font-semibold hover:underline">
                  <ShoppingCart size={14} />
                  {cartCount} · {currency} {cartTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </button>
              )}
            </div>
            ...
            {filteredListings.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-gray-400 gap-3">
                ...
                <button onClick={() => { setFilters(DEFAULT_FILTERS); setSearch(""); }} className="text-sm text-green-600 hover:underline">
                  {t("noListingsClear")}
                </button>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {filteredListings.map((listing) => (
                  <ProduceCard key={listing.id} listing={listing} onAddToCart={handleAddToCart} />
                ))}
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-72 bg-white shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <span className="font-bold text-gray-900">{/* filters title via FilterSidebar */}</span>
              <button onClick={() => setMobileSidebarOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 transition" aria-label={tc("close")}>
                <X size={16} />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-5">
              <FilterSidebar filters={filters} onChange={(f) => { setFilters(f); setMobileSidebarOpen(false); }} totalResults={filteredListings.length} />
            </div>
          </div>
        </div>
      )}
